...
The opt-in QEMU probe created the named chardev socket, but the managed Windows disk stayed at `Start boot option` (`800×600`) with both `max_ports=1` and `max_ports=2` configurations. Connecting a host Unix-socket client immediately after QEMU created the chardev did not change the result. A baseline launch without the VirtIO serial device reached the Windows desktop (`1024×768`) on the same disk. `max_ports=1` is invalid for the named port (`Out-of-range port id specified`), while `max_ports=2` is accepted by QEMU but still fails before the Windows desktop. The current evidence rules out the missing host chardev client as the primary cause; this probe remains blocked by the managed Windows ARM disk/device compatibility and must not be enabled for users until a clean health round trip is proven.
