...
- The first OOBE recovery attempts through the HMP monitor `sendkey` path recorded sender evidence but did not dismiss the live folder-selection dialog. New QEMU launches now also attach a QMP socket and `veil-vmctl qemu-sendkey` prefers QMP `send-key` when that socket exists, falling back to HMP only for older launch records.
...
- `qemu-oobe-bypass` now sends a staged key sequence that first dismisses modal dialogs with `esc`, waits longer after `shift-f10`, and then sends `oobe\bypassnro`. The current QMP evidence shows special keys and command-prompt launch working, but QMP letter input still does not reliably type into the elevated Windows command prompt. Treat OOBE bypass as an active harness blocker until a text-input path is proven by screenshot, not only by QMP sender success.

This means Veil can now distinguish "QEMU is missing" from "QEMU and the ISO are present" from "Windows Setup is actively installing on the local blank NVMe VM disk" from "Windows has reached OOBE." The next QEMU milestone is to use the attached external driver media or a proven offline OOBE path to get through OOBE, verify a network path for the guest-agent WebSocket forward, and run the first guest-agent install without bundling Windows keys or media.
