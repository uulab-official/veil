...
- The same live path exposed the next app-runtime blocker: `app-window-proof --wait-seconds 8` now returns a bounded `launchTimeout` while the Windows desktop visibly opens Notepad. The next debug target is Windows agent app-launch direct replies and first-frame event delivery.
