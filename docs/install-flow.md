...
- While the VM is running with native display fallback enabled, Native QEMU Display brings the active QEMU Cocoa window forward. In the default app path there is no second native display window.
