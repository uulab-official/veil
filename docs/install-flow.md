...
- `veil-vmctl qemu-install-status --json` reads the current runtime snapshot and latest QEMU launch record into a metadata-only Windows install status report. It includes boot readiness, install evidence, latest console screenshot path, latest display endpoint when available, planned 1440x900 guest framebuffer evidence, aspect-fit host-window scaling, Retina host-rendering policy, and safe next actions such as `qemu-display-smoke`, `qemu-capture`, `qemu-oobe-bypass`, or `qemu-install-agent`. It does not start, stop, copy, or modify Windows media or virtual disks.
- When install status detects a running QEMU process for the configured disk but no current launch record, it reports that recovery action before installer or preflight blockers. This protects the disk from relaunch attempts and makes the operator sequence explicit: close the existing Windows/QEMU process, then reselect protected media or rerun readiness checks.
