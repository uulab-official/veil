...
- `veil-vmctl qemu-install-status --json` reads the current runtime snapshot and latest QEMU launch record into a metadata-only Windows install status report. It includes boot readiness, install evidence, latest console screenshot path, latest display endpoint when available, and safe next actions such as `qemu-display-smoke`, `qemu-capture`, `qemu-oobe-bypass`, or `qemu-install-agent`. It does not start, stop, copy, or modify Windows media or virtual disks.
