...
- `veil-vmctl qemu-install-agent --json` now provides a single bounded QMP recovery command for a visible Windows desktop: it opens Run, locates `VEIL_AUTO`, and invokes the packaged guest-agent bootstrap script with PowerShell execution-policy bypass. The command records the same key-send evidence as the lower-level QEMU input tools. The same recovery path is available in the macOS app through the Install Guest Agent button and menu bar item while the QEMU console is running.
