...
- The guest-agent start script is now safe to run repeatedly: it reuses an already-running installed `VeilAgent.exe`, waits for the localhost WebSocket port to become reachable, and records `agent.stdout.log` plus `agent.stderr.log` next to the install/start logs.
