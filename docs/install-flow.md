...
- QEMU/HVF attaches the user-provided Windows ISO, the generated automatic install ISO, and the writable NVMe system disk when starting the VM. NVMe is used for the install-time system disk because the Windows 11 Arm installer sees it without a separate VirtIO storage driver. The QEMU network adapter is currently `e1000` instead of VirtIO because the first real OOBE pass did not find an inbox driver for `virtio-net-pci`.
...
- `veil-vmctl qemu-capture` refreshes the latest running QEMU launch screenshot through the recorded monitor socket without sending a quit command, so the app and diagnostics can follow long Windows setup/OOBE runs.
