...
- `veil-vmctl qemu-install-agent --json [--wait-seconds 30]` now provides a single bounded QMP recovery command for a visible Windows desktop: it opens the Start menu with `Ctrl+Esc`, searches for Run, locates `VEIL_AUTO`, and invokes the packaged guest-agent bootstrap script with PowerShell execution-policy bypass. The command records the same key-send evidence as the lower-level QEMU input tools and then waits for the forwarded guest-agent health endpoint so failed attempts produce actionable next steps instead of a misleading key-send-only success. The same recovery path is available in the macOS app through the Install Guest Agent button and menu bar item while the QEMU console is running.
