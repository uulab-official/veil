...
- `veil-vmctl qemu-install-agent --json [--wait-seconds 30]` now provides a single bounded QMP recovery command for a visible Windows desktop: it taps the Start button through QMP pointer input, searches for Run, locates `VEIL_AUTO`, and invokes the packaged guest-agent bootstrap script with PowerShell execution-policy bypass. The command records the Start activation tap plus key-send evidence and then waits for the forwarded guest-agent health endpoint so failed attempts produce actionable next steps instead of a misleading key-send-only success. The same recovery path is available in the macOS app through the Install Guest Agent button and menu bar item while the QEMU console is running, with the previous `Ctrl+Esc` path kept as a keyboard fallback for older launch records.
