...
- Veil now also follows UTM's AArch64 Secure Boot variable-store clue by discovering `edk2-arm-secure-vars.fd` from the user-local Veil firmware cache or UTM.app, preferring it for new Windows 11 Arm profiles, replacing an existing pre-install `uefi-vars.fd` when upgrading to secure vars, and padding small templates to QEMU's 64 MiB pflash backend size.
