...
- `veil-vmctl qemu-install-agent --json [--wait-seconds 30]` now provides a single bounded QMP recovery command for a visible Windows desktop: it focuses the desktop through QMP pointer input, opens Run with the Windows key, scans attached drive letters for `Veil Guest Agent\Install Veil Agent.cmd`, and invokes that short launcher. The command records the desktop activation tap plus key-send evidence and then waits for the forwarded guest-agent health endpoint so failed attempts produce actionable next steps instead of a misleading key-send-only success.
