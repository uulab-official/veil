...
- Veil's automatic install ISO now includes the staged `Veil Guest Agent` bundle when it exists next to `Autounattend.xml`, so a future Windows desktop boot can access the installer from the attached `VEIL_AUTO` media instead of relying on an unavailable live macOS folder share. The current installer still requires a .NET SDK inside Windows; removing that dependency or shipping a pre-published self-contained agent bundle is the next guest-agent installation blocker.
