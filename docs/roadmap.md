...
8. v1.5 "Daily Use" progress (2026-07-07): Retina-aware capture, multi-window discovery, app icons, and drag-and-drop file open are all shipped and live-verified (see `docs/checklists/2026-07-07-dpi-aware-capture.md`, `2026-07-07-multi-window-discovery.md`, `2026-07-07-real-app-icons.md`, `2026-07-07-drag-and-drop.md`). Frame-latency tuning (the 250ms GDI-polling half of "better frame latency") remains open, needing its own capture-backend investigation (GDI vs. Windows.Graphics.Capture/DXGI). The two remaining unstarted v1.5 items — Windows notifications and printer bridge — now have feasibility research done: Windows notifications is feasible via a signed sparse package granting package identity to the existing unpackaged guest agent (real new cost: code signing + a certificate-trust step in the install flow); printer bridge is feasible with no new QEMU infrastructure, since the guest already reaches the host over QEMU's existing user-mode/SLIRP networking, so Windows can add the host's shared printer as a plain IPP network printer. See `docs/checklists/2026-07-07-notifications-and-printer-feasibility.md` for the full research and recommended next spikes for each.
