...
9. One-click installed-VM optimization (2026-08-05): the app now offers one consented `Optimize Windows` action for an installed QEMU guest that lacks healthy integration. It downloads and validates official UTM Guest Tools, safely restarts to attach read-only support media, installs display support and the Veil guest agent, waits for reconnection, and checks live framebuffer dimensions. Finder-launched builds also discover Veil's managed QEMU and `swtpm` runtime automatically. Automated regression, signed app installation, and the real combined-confirmation screen are verified; accepting the terms and recording the current VM's complete restart/reconnect/resolution result remains the next live gate. See `docs/checklists/2026-08-04-built-app-live-install-pass.md`.
