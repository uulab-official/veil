...
The report includes named checks for VM profile, installer media, automatic install media, system disk, QEMU executable, Arm UEFI firmware plus writable `uefi-vars.fd`, Secure Boot firmware capability, `swtpm` TPM 2.0 emulator, and HVF command plan. Blocked reports must include next actions that a contributor can follow without guessing.
