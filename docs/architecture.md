...
- The host-side `veil-vmctl mvp-proof --json --app-id winapp_notepad` command is the single release gate for the first success demo: it waits for the guest agent and then runs the Coherence proof. Connected reports must include `guestAgentWait` plus `windowsAppCoherenceProof`; unavailable reports keep recovery guidance but do not count as release proof. The harness supports `node harness/mvp-proof/src/validate-mvp-proof.mjs --require-proved` for release gates and omits that flag only when validating recovery-report shape.
