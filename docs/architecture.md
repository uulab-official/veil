...
- A notification streamer boundary is wired into the WebSocket broadcast loop and emits `notification.received` events after validation/deduplication. The default listener remains disabled until the signed sparse package identity and Windows `UserNotificationListener` consent flow are live-verified.
