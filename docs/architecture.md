...
VM installation media, disk, provider, resource, and integration details live in one item-routed Windows Settings sheet. The main display keeps only the current Windows action and a Settings entry point; resource-changing controls are locked while the VM is starting, running, suspended, or refreshing so inspection cannot accidentally mutate a live session.
...
The guest-agent endpoint is selected independently but consistently with the runtime provider. QEMU owns the proved loopback `hostfwd` route. Apple Virtualization currently has no proved host-to-guest WebSocket route, so the host does not poll QEMU's loopback endpoint while Apple is active and does not imply that app-window integration will follow from console boot alone. An explicit `VEIL_AGENT_URL` is the extension point for development transports until a supported direct or reverse channel is implemented.
