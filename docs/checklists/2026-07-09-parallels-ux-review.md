...
- [x] Replace review handoff helper copy from command-line wording to Review Evidence language, so ready-for-review states stay product-facing inside the launcher.
...
- [x] Promote stale display recovery in the Dock menu and disable direct app-open items while recovery is required, matching the menu-bar behavior and avoiding dead app-launch clicks.
...
- [x] Keep menu-bar primary action routing and symbols behind one tested route table, so top-bar recovery controls do not drift from `menuBarIntegration.primaryActionId`.
...
- [x] Add `invalidScreenshotCount` and a `Replace Review Screenshots` next step to review cards, so invalid evidence asks for replacement instead of generic attachment.
...
- [x] Add structured `invalidScreenshotFiles` to review verification reports, so existing but unusable files expose whether they are unreadable, not valid PNGs, or below the minimum dimensions.
- [x] Add structured `invalidCaptureSteps` and `nextInvalidCaptureStep` to review verification reports, so invalid screenshots point to the exact replacement capture command instead of only listing the bad file.
- [x] Add `screenshotEvidenceSummary` to review verification reports, so app/UI automation can read one compact share-readiness state with valid, missing, invalid, pending counts and the next capture command.
- [x] Add structured `missingCaptureSteps` and `nextMissingCaptureStep` to `app-runtime-review-verify`, so live VM review passes can advance one screenshot at a time with the exact expected file, instruction, and supporting command.
...
- [x] Require a saved `mvp-proof.json` in each review evidence folder, generated by the manifest's `mvp-proof --require-proved --output` command and verified as `windowsMVPProof` with `status=proved`, so visual review screenshots and the actual Windows app loop proof travel together.
...
- [x] Align the macOS app `Prepare Review Evidence` manifest and README with the CLI evidence contract by including the same `mvp-proof.json` file, proved app-check command, and app-window capture supporting command.
- [x] Remember the latest app-created Review Evidence folder and save subsequent MVP app checks to its fixed `mvp-proof.json`, so the app's Prepare Review Evidence -> Check Windows App path fills the same folder that verification checks.
- [x] Persist the latest app-created Review Evidence folder across app launches and clear stale saved folders, so Check Windows App keeps saving to the same `mvp-proof.json` after reopening Veil.
...
- [x] Persist app-window counts in restore intent and replay them on reconnect, so two Notepad windows restore as two app launches instead of collapsing to one app id.
- [x] Surface restorable window count in Dock/status/menu copy, so one app with two restorable windows shows `R2` and reconnects as two Windows app windows.
