...
- [x] 라이브 Windows 위의 전체 도구 캡슐을 제거하고 앱 보기 복귀 버튼 하나만 남긴다.
- [x] `⇧⌘A`로 Windows 데스크톱과 앱 보기를 전환할 수 있다.
- [x] 일반 최소 창 크기를 `900×620`, 제한된 화면의 지원 하한을 `820×560`으로 낮춘다.
- [x] 앱 Dock 목록을 가로 `LazyHStack`으로 구성해 앱 수가 늘어날 때도 필요한 항목만 배치한다.
- [x] `RuntimeDisplaySelectionTests` 4개가 통과한다.
- [x] `RuntimeWorkspacePresentationPolicyTests` 3개가 통과한다.
- [x] 전체 Swift 테스트 385개 / 26 suites가 통과한다.
...
- [ ] 실제 `820×560` 창에서 앱 Dock 가로 스크롤과 상태 배지가 잘리지 않는지 확인한다.
