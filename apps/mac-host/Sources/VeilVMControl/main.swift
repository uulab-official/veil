...
struct VMControlArguments {
    enum AppRuntimeAction: String, Equatable, Codable {
        ...
        case maintainFrameStreams = "maintain-frame-streams"
        case restore
    ...
    static func parse(_ arguments: [String]) throws -> VMControlArguments {
        ...
        if command == "multi-app-proof" {
            let appIds = appIdsArgument(from: arguments) ?? WindowsAppRuntimeProofCoverageDefaults.targetAppIds
            guard !appIds.isEmpty else {
                throw VMControlError.missingMultiAppProofAppIds
            }
            return VMControlArguments(
                command: .multiAppProof(
                    json: arguments.contains("--json"),
                    appIds: appIds,
        ...
        if command == "printer-bridge-plan" {
            return VMControlArguments(
                command: .printerBridgePlan(
                    json: arguments.contains("--json"),
    ...
    private static func appIdsArgument(from arguments: [String]) -> [String]? {
        guard let appIdsValue = stringArgument(named: "--app-ids", from: arguments) else {
            return nil
        }
...
struct WindowsPrinterBridgePlan: Codable, Equatable {
    var kind: String = "windowsPrinterBridgePlan"
    ...
    var macOSSharingHint: String
    var windowsAddPrinterCommand: String
    var windowsVerifyPrinterCommand: String
    var manualTestPageInstruction: String
...
struct WindowsMultiAppProofReport: Codable, Equatable {
    var kind: String = "windowsMultiAppProof"
    var endpoint: String
    var provedAt: Date
    ...
    var results: [WindowsMultiAppProofResult]
    var nextActions: [String]
}

struct WindowsMultiAppProofResult: Codable, Equatable {
    var appId: String
    var status: String
    var proofKind: String?
...
struct WindowsMultiAppProofLatencySummary: Equatable {
    var health: String
...
struct VeilVMControl {
    ...
    private static func printAppRuntimeStatus(json: Bool, demo: Bool) async throws {
        ...
        print("Daily Use printer plan: \(report.dailyUseReadiness.printerBridgePlanCommand)")
    ...
    private static func runAppRuntimeAction(
        ...
        case .restartFrameStream:
            if let restartWindowId = windowId,
               !restartWindowId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                if await model.restartFrameSubscription(windowId: restartWindowId) {
                    restartedFrameWindowIds = [restartWindowId]
                }
                resolvedWindowId = restartWindowId
            } else {
                restartedFrameWindowIds = await model.restartStaleFrameSubscriptions()
                resolvedWindowId = restartedFrameWindowIds.last
            }
            ...
            accepted = !restartedFrameWindowIds.isEmpty
        case .recoverWindowCapture:
            if let recoverWindowId = windowId,
               !recoverWindowId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                if await model.recoverFrameCapture(windowId: recoverWindowId) {
                    recoveredFrameWindowIds = [recoverWindowId]
                }
                resolvedWindowId = recoverWindowId
            } else {
                recoveredFrameWindowIds = await model.recoverEscalatedFrameCaptures()
                resolvedWindowId = recoveredFrameWindowIds.last
            }
            if let foregroundSession = model.mirrorSessions.last {
                foregroundWindowId = foregroundSession.id
                foregroundWindowTitle = foregroundSession.window.title
        ...
        case .maintainFrameStreams:
            let maintenance = await model.maintainStaleFrameStreams()
            restartedFrameWindowIds = maintenance.restartedFrameWindowIds
            recoveredFrameWindowIds = maintenance.recoveredFrameWindowIds
            reopenRequestedWindowIds = maintenance.reopenedWindows.map(\.requestedWindowId)
            reopenedWindows = maintenance.reopenedWindows.map(\.launch.window)
            if let foregroundWindow = reopenedWindows.last {
                resolvedAppId = foregroundWindow.appId
                resolvedWindowId = foregroundWindow.windowId
                foregroundWindowId = foregroundWindow.windowId
                foregroundWindowTitle = foregroundWindow.title
            } else if let foregroundSession = model.mirrorSessions.last {
                resolvedAppId = foregroundSession.window.appId
                resolvedWindowId = foregroundSession.id
                foregroundWindowId = foregroundSession.id
                foregroundWindowTitle = foregroundSession.window.title
            }
            accepted = maintenance.didPerformMaintenance
            ...
            if !demo,
               ...
               beforeStatus.localRuntime.requiresGuestToolsMediaRebuild != true {
                ...
                if agentRepair?.status == .connected {
                    ...
                    launch = result?.launch
                    window = result?.window
                    resolvedAppId = result?.window.appId ?? model.pendingLaunchAppId ?? beforeStatus.pendingLaunchAppId
                    resolvedWindowId = result?.window.windowId
                    foregroundWindowId = result?.window.windowId
                    foregroundWindowTitle = result?.window.title
        ...
        let actionLaunchPlan = action == .launch || action == .fulfillPending || action == .repairAgent ? status.launchPlan : nil
        ...
        if let agentRepair = report.agentRepair {
        ...
        if !report.restartedFrameWindowIds.isEmpty {
            print("Restarted app screen streams: \(report.restartedFrameWindowIds.joined(separator: ", "))")
        }
    ...
    private static func nextActions(
        ...
        if accepted {
            ...
            case .restartFrameStream:
                return compactActions([
                    "Run `veil-vmctl app-runtime-status --json` to confirm frameStreamStatus returns to waitingForFirstFrame or fresh.",
                    proofNextAction(from: status.proofPlan)
                ])
            case .recoverWindowCapture:
                return compactActions([
                    "Run `veil-vmctl app-runtime-status --json` to confirm frameStreamRecoveryEscalated is false.",
                    proofNextAction(from: status.proofPlan),
            ...
            case .maintainFrameStreams:
                return compactActions([
                    "Confirm mirrored Windows app windows are visible as macOS windows.",
                    proofNextAction(from: status.proofPlan),
                    "Run `veil-vmctl app-runtime-status --json` to confirm stale app screens are waiting for frames or fresh."
                ])
            case .restore:
                return compactActions([
            ...
            case .repairAgent:
                return compactActions([
                    status.pendingLaunch.isQueued
                        ? status.launchPlan.recommendedLaunchCommand.map { "Run `\($0)` to continue the queued Windows app launch." }
                        : nil,
        ...
        if action == .restartFrameStream {
            return [
                "Run `veil-vmctl app-runtime-status --json` and check mirrorSessions[].frameStreamStatus.",
                "Run `veil-host-probe --diagnose-agent` if the app screen stream cannot be restarted."
            ]
        }

        if action == .recoverWindowCapture {
            return [
        ...
        if action == .maintainFrameStreams {
            return [
                "Run `veil-vmctl app-runtime-status --json` and check mirrorSessions[].frameStreamStatus.",
                "Run `veil-host-probe --diagnose-agent` if paused app screens cannot be maintained."
            ]
        }
        ...
        if action == .repairAgent {
            ...
            return compactActions([
                status.launchPlan.recommendedRepairCommand.map { "Run `\($0)` to retry the in-app guest-agent repair path." },
                "Run `veil-host-probe --diagnose-agent` to inspect host-forward TCP and WebSocket health.",
                "Run `veil-vmctl app-runtime-status --json` before retrying app launch or reconnect-restore."
            ])
        }
    ...
    private static func proveMultipleApps(
        json: Bool,
        appIds: [String],
        waitSeconds: Int,
        ...
        let url = URL(string: endpoint) ?? URL(string: "ws://127.0.0.1:18444")!
        let boundedWaitSeconds = min(max(waitSeconds, 1), 60)
        let targetAppIds = orderedUniqueAppIds(appIds)
        guard !targetAppIds.isEmpty else {
            throw VMControlError.missingMultiAppProofAppIds
        }

        let diagnosticsDirectory = outputDirectoryPath.map(URL.init(fileURLWithPath:))
        ...
        let stamp = proofFileTimestamp()
        let aggregateReportURL = recommendedProofDirectory
            .appendingPathComponent("multi-app-proof-\(stamp).json")
        var results: [WindowsMultiAppProofResult] = []

        for appId in targetAppIds {
            let outputURL = coherenceProofDirectory
                .appendingPathComponent("\(proofFileComponent(appId))-coherence-proof-\(stamp).json")
            let transport = URLSessionWebSocketTransport(url: url)
            let client = VeilHostClient(transport: transport)
            ...
            do {
                ...
                proof.savedProofPath = outputURL.path
                try writeProof(proof, to: outputURL)

                let latency = multiAppLatencySummary(for: proof)
                results.append(
                    WindowsMultiAppProofResult(
                        appId: appId,
                        status: "proved",
                        proofKind: "coherence",
                        proofPath: outputURL.path,
        ...
        if provedAppCount == targetAppIds.count {
            coverageHealth = "complete"
        } else if provedAppCount > 0 {
            coverageHealth = "partial"
        } else {
            coverageHealth = "missing"
        }

        let nextActions = multiAppProofNextActions(
            coverageHealth: coverageHealth,
            aggregateReportPath: aggregateReportURL.path,
            failedResults: results.filter { $0.status == "failed" }
        )
        let report = WindowsMultiAppProofReport(
            endpoint: endpoint,
            provedAt: Date(),
            proofDirectory: coherenceProofDirectory.path,
            aggregateReportPath: aggregateReportURL.path,
            appIds: targetAppIds,
            targetAppIds: WindowsAppRuntimeProofCoverageDefaults.targetAppIds,
            waitSeconds: boundedWaitSeconds,
            proofKind: "coherence",
    ...
    private static func orderedUniqueAppIds(_ appIds: [String]) -> [String] {
        var seen = Set<String>()
        var result: [String] = []
        for appId in appIds {
    ...
    private static func proofFileTimestamp() -> String {
    ...
    private static func multiAppLatencySummary(
    ...
    ) -> WindowsMultiAppProofLatencySummary {
        ...
        return WindowsMultiAppProofLatencySummary(
            health: health,
    ...
    private static func multiAppProofNextActions(
        ...
        failedResults: [WindowsMultiAppProofResult]
    ) -> [String] {
        var actions = [
    ...
    private static func printPrinterBridgePlan(
        json: Bool,
        ...
        print("Windows printer bridge: \(plan.mode)")
        print("IPP endpoint: \(plan.ippEndpoint)")
        print("Windows printer: \(plan.windowsPrinterName)")
        print("macOS setup: \(plan.macOSSharingHint)")
        print("Windows command: \(plan.windowsAddPrinterCommand)")
        print("Verify: \(plan.windowsVerifyPrinterCommand)")
        print("Test page: \(plan.manualTestPageInstruction)")
        print("Next actions:")
    ...
    private static func makePrinterBridgePlan(
        sharedPrinterName: String?,
        windowsPrinterName: String?
    ) -> WindowsPrinterBridgePlan {
        let sharedPrinter = normalizedPrinterBridgeName(
            sharedPrinterName,
            fallback: "<shared-printer-name>"
        )
        let windowsPrinter = normalizedPrinterBridgeName(
            windowsPrinterName,
            fallback: "Veil Mac Printer"
        )
        let endpoint = printerBridgeEndpoint(sharedPrinterName: sharedPrinter)
        let addCommand = "Add-Printer -Name \(powershellSingleQuoted(windowsPrinter)) -IppURL \(powershellSingleQuoted(endpoint))"
        let verifyCommand = "Get-Printer -Name \(powershellSingleQuoted(windowsPrinter)) | Format-List Name,Type,PrinterStatus"

        return WindowsPrinterBridgePlan(
            generatedAt: Date(),
            mode: WindowsAppRuntimePrinterBridgeDefaults.recommendedAction,
            hostAddress: "10.0.2.2",
            ippPort: 631,
            sharedPrinterName: sharedPrinter,
            windowsPrinterName: windowsPrinter,
            ippEndpoint: endpoint,
            macOSSharingHint: "Enable macOS Printer Sharing, then share the target printer as \(sharedPrinter).",
            windowsAddPrinterCommand: addCommand,
            windowsVerifyPrinterCommand: verifyCommand,
            manualTestPageInstruction: "From Windows Settings, open Printers & scanners, select \(windowsPrinter), and print a test page.",
            setupSteps: [
                "On macOS, enable Printer Sharing and share the printer name shown in sharedPrinterName.",
                "Start the Veil Windows VM on the default QEMU user-mode network.",
                "In an elevated Windows PowerShell session, run windowsAddPrinterCommand.",
                "Open Windows Printers & scanners and confirm the new printer appears."
            ],
            verificationSteps: [
                "Run windowsVerifyPrinterCommand and confirm the printer is present.",
                "Print a Windows test page and save screenshot or paper/PDF evidence before claiming printer bridge support.",
                "Run `veil-vmctl app-runtime-status --json` and confirm dailyUseReadiness.printerBridgeRecommendedAction remains manual-ipp-experiment until live proof exists."
            ],
            limitations: [
                "This is a manual IPP experiment, not automatic Parallels-style printer provisioning yet.",
                "It depends on macOS Printer Sharing, the Windows IPP print path, and the selected printer driver.",
                "Veil must not claim printer bridge support until a real Windows test page proof is saved."
            ],
            nextActions: [
                "Run `veil-vmctl printer-bridge-plan --json --shared-printer <shared-printer-name>` with the actual macOS shared printer name.",
                "Run the generated Add-Printer command inside Windows after the guest is installed and networked.",
                "Save the successful Windows test-page evidence, then promote this from manual experiment to app-guided printer setup."
            ]
        )
    }

    private static func normalizedPrinterBridgeName(_ value: String?, fallback: String) -> String {
        let trimmed = value?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? fallback : trimmed
    }

    private static func printerBridgeEndpoint(sharedPrinterName: String) -> String {
        if sharedPrinterName == "<shared-printer-name>" {
            return WindowsAppRuntimePrinterBridgeDefaults.endpointTemplate
        }

        let allowed = CharacterSet(charactersIn: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~")
        let encoded = sharedPrinterName.addingPercentEncoding(withAllowedCharacters: allowed) ?? sharedPrinterName
        return "http://10.0.2.2:631/printers/\(encoded)"
    }
