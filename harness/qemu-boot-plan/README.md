...
The plan is intentionally a dry run. It describes the QEMU/HVF command Veil would need for a Windows 11 Arm install path, including Arm UEFI firmware, ISO boot order, lock-safe read-only installer media, and local display/input devices. The validator never launches QEMU and never touches the installer or disk.
