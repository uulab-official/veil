...
Expected current output on the test Mac: `qemu smoke valid`; the JSON currently reports `uefiShell` plus a `.ppm` `consoleScreenshotPath` and recovery `nextActions`, which means QEMU reaches Arm UEFI but Windows Setup is not proven.
