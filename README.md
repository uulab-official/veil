...
The profile can reference a user-provided installer image, an optional external driver ISO, and a virtual disk path, or use Veil's default blank disk file. Veil checks that the stored paths still point to local files, that installer media looks like a bootable ISO instead of a disk-image import, that protected Downloads-folder media has been selected through the app file picker so macOS grants a security-scoped bookmark, that the macOS shared folder exists, and that the profile targets Windows Arm with usable CPU, memory, and disk settings before marking the profile boot-ready. Pressing Start in the macOS app builds the active local runtime plan and starts QEMU/HVF in headless single-window preview mode, so the main Veil window remains the product surface while setup evidence refreshes there. Set `VEIL_USE_NATIVE_QEMU_DISPLAY=1` before launching the app to opt into the temporary native QEMU display fallback. Pressing Stop stops the active VM process. It still does not validate Windows media contents, include Windows or driver media, or bypass licensing.

When native display fallback is enabled, the Native Display action can bring the QEMU Cocoa window forward while the VM is running. The current Windows path is still a feasibility spike: a running local provider process plus screenshots does not yet prove that every Windows 11 Arm ISO will present a fully interactive embedded installer. UTM-level Windows reliability remains a roadmap item, not a completed claim.
...
This starts a local Cocoa QEMU window from the prepared Windows Arm profile, brings it forward, and writes logs under `~/Library/Application Support/Veil/Diagnostics/QEMU Launch`. App-launched QEMU sessions default to `-display none`, record their display mode in `qemu-launch-latest.json`, and still capture a `qemu-console-*.png` VM-display screenshot path for the macOS setup screen when QEMU exposes a frame. Veil still uses user-provided Windows media and does not bundle QEMU, firmware, Windows images, product keys, or activation material.
