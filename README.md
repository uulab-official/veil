...
On first launch, `Download Windows 11` opens Microsoft's official Windows 11 Arm64 page in a background WebKit view, selects the current Arm64 edition and the Mac's preferred Windows language, requests Microsoft's temporary latest ISO link, and starts the download without asking the user to operate the webpage. Before writing, Veil rejects failed HTTP responses, implausibly short payloads, and downloads that do not fit with a 2 GB storage reserve. It saves the ISO under its local Application Support directory and streams a SHA-256 calculation without loading the multi-gigabyte file into memory; VM preparation continues only when that value matches the selected language hash published on Microsoft's page. Failed, mismatched, and cancelled downloads are removed. Veil then prepares the default profile, sparse disk, shared folder, and automatic install media, and starts the local VM so Windows Setup can appear in the embedded display. `Show Microsoft Page` exposes the official controls if Microsoft's page changes, while `Use Existing ISO` keeps the local file-picker path available. If download, integrity validation, media validation, or a host prerequisite fails, Veil stays in the setup flow and shows the blocker instead of starting an unusable VM. The separate Windows Settings sheet owns configuration-only installer, driver, disk, provider, and resource actions without crowding the main display.
...
Guest-agent connectivity is provider-aware. QEMU uses the proved loopback-only `ws://127.0.0.1:18444` host forward. Apple Virtualization does not currently invent or probe that QEMU endpoint: its console remains usable, while Windows app-window integration is reported unavailable until a routable transport is configured. Developers with a real reachable route can set `VEIL_AGENT_URL` to an explicit `ws://` or `wss://` endpoint; malformed or non-WebSocket overrides are rejected before a connection attempt.

The profile can reference a user-provided installer image, an optional external driver ISO, and a virtual disk path, or use Veil's default blank disk file. Before Windows is installed, Veil checks that the stored paths still point to local files, that installer media looks like a bootable ISO instead of a disk-image import, that protected Downloads-folder media has been selected through the app file picker so macOS grants a security-scoped bookmark, that the macOS shared folder exists, and that the profile targets Windows Arm with usable CPU, memory, and disk settings before marking the profile boot-ready. After Windows is installed on the VM disk, the Windows installer ISO is no longer required for normal boot and is not attached by the QEMU/HVF plan. Pressing Start in the macOS app builds the active local runtime plan and starts QEMU/HVF in headless single-window preview mode, so the main Veil window remains the product surface while setup evidence refreshes there. Set `VEIL_USE_NATIVE_QEMU_DISPLAY=1` before launching the app to opt into the temporary native QEMU display fallback. Pressing Stop stops the active VM process. It still does not validate Windows media contents, include Windows or driver media, or bypass licensing.

When native display fallback is enabled, the app menu and menu bar expose an Open Recovery Display action that can bring the QEMU Cocoa window forward while the VM is running. The main Veil window does not show this as a normal runtime control; the default product path is the embedded display surface. The current Windows path is still a feasibility spike: a running local provider process plus embedded display evidence does not yet prove that every Windows 11 Arm ISO will present a fully interactive installer. UTM-level Windows reliability remains a roadmap item, not a completed claim.
